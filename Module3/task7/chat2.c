...
void ParticipantSecond(struct mq_attr config)
{
    mqd_t send_fd, recv_fd;
    char text_buffer[MSG_MAX];
    unsigned int priority_val;

    recv_fd = mq_open(PATH1, O_RDONLY);
    if (recv_fd == (mqd_t)-1) 
    {
        perror("mq_open recv");
        exit(1);
    ...
    send_fd = mq_open(PATH2, O_WRONLY);
    if (send_fd == (mqd_t)-1) 
    {
        perror("mq_open send");
        exit(1);
...
int main() 
{
    ...
    printf("Chat participant 2 started\n"); 
    ParticipantSecond(config);
    printf("Chat ended\n");
    return 0;
}