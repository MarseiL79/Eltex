...
int main() {
    ...
    if (loadContacts() == 0) {
        getAllContacts(&contactsCount);
        printf("Contacts loaded successfully... (total: %d)", contactsCount);
    }
    else {
        printf("No saved contacts or load error...");
    }
    ...
    do {
        ...
        switch (choice) {
            ...
            case 5:
                getAllContacts(&contactsCount);
                if (saveContacts() == 0) {
                    printf("Contacts saved successfully. Total: %d\n", contactsCount);
                }
                else {
